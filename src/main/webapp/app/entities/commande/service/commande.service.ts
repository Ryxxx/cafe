...
export class CommandeService {
  ...
  protected convertDateFromClient(commande: ICommande): ICommande {
    return Object.assign({}, commande, {
      dateCreation: commande.dateCreation?.isValid() ? commande.dateCreation.format(DATE_FORMAT) : undefined,
      dateModification: commande.dateModification?.isValid() ? commande.dateModification.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((commande: ICommande) => {
        ...
        commande.dateModification = commande.dateModification ? dayjs(commande.dateModification) : undefined;
      });
    }
    return res;
  }
